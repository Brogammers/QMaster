        ...
        <table className="w-full">
          <thead>
            <tr className={`border-b ${isDarkMode ? 'border-white/10' : 'border-slate-300'}`}>
              ...
              <th className={`px-6 py-3 text-left text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Locations</th>
          ...
          <tbody>
            {filteredPartners.map((partner) => (
              <>
                <tr 
                  key={partner.id} 
                  className={`border-b ${isDarkMode ? 'border-white/10 hover:bg-white/[0.02]' : 'border-slate-300 hover:bg-slate-50/50'} cursor-pointer`}
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {expandedPartnerId === partner.id ? <FaChevronUp className="w-3 h-3" /> : <FaChevronDown className="w-3 h-3" />}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className={`px-6 py-4 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                    {partner.locations.length} {partner.locations.length === 1 ? 'location' : 'locations'}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-sm ${
                ...
                <AnimatePresence>
                  {expandedPartnerId === partner.id && (
                    <motion.tr
                      ...
                      className={`${isDarkMode ? 'bg-white/[0.02]' : 'bg-slate-50/50'}`}
                    >
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className={`${isDarkMode ? 'bg-black/20' : 'bg-slate-100'}`}>
                                ...
                                <th className={`px-4 py-2 text-left text-sm font-medium ${isDarkMode ? 'text-white/70' : 'text-slate-600'}`}>Maps</th>
                              </tr>
                            </thead>
                            <tbody>
