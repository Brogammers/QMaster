...
export default function SearchItem(props: ExtendedSearchItemProps) {
  ...
  const handlePress = () => {
    if (onPress) {
      ...
      linkTo(
        `/Partner?brandName=${encodeURIComponent(
          title
  ...
  return (
    <TouchableOpacity
      className={`w-full ${isAccount ? "py-4 px-4" : "py-3 px-4"} ${
        isDarkMode ? "border-baby-blue/20" : "border-lite-grey"
      } ${!isAccount && "border-b border-gray-200"}`}
      onPress={handlePress}
    >
      <View className={`flex-row items-center justify-between w-full`}>
        {isAccount ? (
          <View className="flex-row items-center">
            ...
            />
            <Text
              className={`ml-5 text-xl font-medium ${
          ...
          <View className="flex-row items-center">
            ...
            <Text
              className={`ml-5 text-base font-medium ${
                isDarkMode ? "text-baby-blue" : "text-gray-800"
              }`}
            >
              {title}
            </Text>
